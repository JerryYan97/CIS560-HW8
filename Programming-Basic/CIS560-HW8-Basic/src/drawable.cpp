...
void Drawable::generateInfluJointID()
{
    influJointsIDBound = true;
    mp_context->glGenBuffers(1, &bufInfluJointsID);
}
...
bool Drawable::bindInfluJointID()
{
    if(influJointsIDBound)
    {
        mp_context->glBindBuffer(GL_ARRAY_BUFFER, bufInfluJointsID);
    }
    return influJointsIDBound;
}
